...
> **⚠️ Important Note:**
> Google has removed the free tier for the Gemini API. Currently, this affects the **Pro model** (gemini-2.5-pro), which now requires a paid API key. The **Flash model** (gemini-2.5-flash/gemini-3-flash-preview) may still work with free tier access, but please be aware that API pricing and availability may change.
